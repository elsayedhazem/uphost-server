...
class DestinationManager():
	...
	def manage_destination(self):
		...
		if self.new_destination_features:
			print(self.new_destination_features)
	...
	def __extract_pricing_features(self):
		prices = { listing_id: features.get("pricing", {}).get("totalPrice", {}).get("amount", None) for listing_id, features in self.listings_features.items() }
		...
		max_price_per_guest = { list(prices_per_guest.keys())[list(prices_per_guest.values()).index(max_price_per_guest)] : max_price_per_guest }
		
		min_price_per_guest = min(list(prices_per_guest.values()))
		min_price_per_guest = { list(prices_per_guest.keys())[list(prices_per_guest.values()).index(min_price_per_guest)] : min_price_per_guest }

		avg_price_per_guest = sum(list(prices_per_guest.values())) // len(list(prices_per_guest.values()))
		return {
	...
	def __extract_booking_rates(self, occupancyPercentages):
		nonzero_occupancies = { k:v for k,v in occupancyPercentages.items() if v }
		...
		current_booking_percentages = { k: v / n_listings for k,v in number_of_bookings_at_dates.items() }
		
		return current_booking_percentages
			

	def __extract_new_features(self):
		occupancyPercentages = { listing_id: features.get("occupancyPercentage", 0) for listing_id, features in self.listings_features.items() }
		...
		self.new_destination_features["bookingRates"] = self.__extract_booking_rates(occupancyPercentages)
		self.new_destination_features["avgOccupancy"] = sum(list(occupancyPercentages.values())) // len(list(occupancyPercentages.values()))
